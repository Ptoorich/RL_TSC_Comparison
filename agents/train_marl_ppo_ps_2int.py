...
class SharedModelWrapper(gym.Env):
    ...
    gym interface. The shared model sees both agents' experiences
...
# TL2 uses the same shared model via shared_model reference
wrapper = SharedModelWrapper(marl_env, primary_agent="tl1")

# ── Single shared PPO model — same hyperparameters as all other runs ───
shared_model = PPO(
    policy          = "MlpPolicy",
    env             = wrapper,
...
print("=== MARL PPO with Parameter Sharing — 2 Intersection Network ===")
print(f"Agents:          {config['agent_ids']}")
print(f"Obs size:        {marl_env.obs_size} (local + normalised ID)")
print(f"  TL1 ID:        {marl_env.agent_index['tl1']:.4f}")
print(f"  TL2 ID:        {marl_env.agent_index['tl2']:.4f}")
print(f"Action space:    Discrete(3) per agent")
print(f"Shared model:    ONE PPO network for both agents")
...
checkpoint = CheckpointCallback(
    ...
    name_prefix = "marl_ppo_ps_2int"
)

shared_model.learn(
    total_timesteps = 260000,
    callback        = checkpoint,
    tb_log_name     = "marl_ppo_ps_2int"
)

# ── Save ───────────────────────────────────────────────────────────────
shared_model.save("models/marl_ppo_ps_2int_260k")
print("\nTraining complete.")
print("Saved: models/marl_ppo_ps_2int_260k.zip")
