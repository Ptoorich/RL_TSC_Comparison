...
# Run this script from agents/ — matches train_marl_dqn_ind_4int.py's config.
config = {
...
# ── Load each agent's final (independent) model ──────────────────────────
models = {
    agent: DQN.load(f"models/marl_dqn_ind_4int_{agent}_260k")
    for agent in config["agent_ids"]
}
env = SumoMARLEnvInd(config)
...
print("=== Evaluating MARL DQN (independent) — 4 Intersection Network ===")
...
for ep, seed in enumerate(SEEDS):
    ...
    while not terminated:
        actions = {
            agent: int(models[agent].predict(
...
with open("results/marl_dqn_ind_4int_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("\nResults saved to results/marl_dqn_ind_4int_results.json")