...
# ── Load trained models ────────────────────────────────────────────────
models = {
    agent: PPO.load(f"models/marl_ppo_{agent}_260k")
    for agent in config["agent_ids"]
}

env = SumoMARLEnv(config)
...
print("=== Evaluating MARL PPO — 2 Intersection Network ===")
...
for ep, seed in enumerate(SEEDS):
    ...
    while not terminated:
        # Each agent acts greedily on its local observation
        actions = {
        ...
        for agent in config["agent_ids"]:
            try:
                import traci
                qs = [traci.lane.getLastStepHaltingNumber(l)
                      for l in config["lane_map"][agent]]
                all_queues.extend(qs)
            except Exception:
                pass
        if all_queues:
            ep_queues.append(np.mean(all_queues))
...
# ── Save ───────────────────────────────────────────────────────────────
with open("results/marl_ppo_2int_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("\nResults saved to results/marl_ppo_2int_results.json")