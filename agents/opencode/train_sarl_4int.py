...
model.learn(
    ...
    callback        = callbacks,
    tb_log_name     = "sarl_ppo_4int",
)

# ── Step 5: Save the trained model ────────────────────────────────────
final_name = (f"sarl_ppo_4int_{TOTAL_TIMESTEPS}"
              if TOTAL_TIMESTEPS != 260000 else "sarl_ppo_4int_260k")
model.save(final_name)
print(f"Model saved as {final_name}.zip")
